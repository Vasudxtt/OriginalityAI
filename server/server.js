...
{
role: "system",
content: "You are an expert academic integrity analyst. Respond with a single valid
},
...
// Actual prompt with real chunk text
const prompt = `Part ${idx + 1} of ${total}. Analyze for plagiarism and AI-generation.
TEXT: """${chunk}"""
Reply with ONLY this JSON:
{"plagiarism_score":<0-100>,"ai_likelihood":"Low|Medium|High","summary":"<2 sentences>","flag
...
// Return a safe fallback so one bad chunk doesn't kill the whole job
return {
plagiarism_score: 50,
...
.slice(0, 15);
return { plagiarism_score, ai_likelihood, summary, flagged_sections, improvement_suggestion
}
...
console.warn(` Chunk ${i + 1} attempt ${attempt} failed (${err.message}). Retr
if (attempt >= 5) {
// Push fallback so job continues
results.push({
plagiarism_score: 50, ai_likelihood: "Medium",
summary: `Chunk ${i + 1} could not be analyzed.`,
flagged_sections: [], improvement_suggestions: [],
});
break;
}
...
console.log(` [${jobId}] Done — score: ${final.plagiarism_score}, AI: ${final.ai_likeli
} catch (err) {
