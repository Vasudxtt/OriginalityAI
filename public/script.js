...
// ── Drag & Drop ────────────────────────────────────────────────────────────────
// Drag over styling
dropZone.addEventListener('dragenter', ev => { ev.preventDefault(); ev.stopPropagation(); dro
dropZone.addEventListener('dragover', ev => { ev.preventDefault(); ev.stopPropagation(); dro
dropZone.addEventListener('dragleave', ev => { ev.preventDefault(); ev.stopPropagation(); dro
...
showToast(' return;
Only PDF and DOCX files are supported.', 'error');
}
...
if (current === 0) {
// Chunk 1 hasn't finished yet — show warming up state
loaderLabel.textContent = total > 0
? 'Warming up — analyzing chunk 1 of ' + total + '…'
: 'Starting analysis…';
// Keep indeterminate bar animating
} else {
loaderLabel.textContent = 'Analyzing chunk ' + current + ' of ' + total + '…';
...
flagList.innerHTML = '';
if (!flags.length) {
flagList.innerHTML = '<p style="font-size:14px;color:var(--text3);padding:8px 0">No phras
} else {
...
suggList.innerHTML = '';
if (!suggs.length) {
suggList.innerHTML = '<p style="font-size:14px;color:var(--text3);padding:8px 0">No speci
} else {
...
const replacement = f.replacement || 'Consider rewriting this phrase in your own authentic
div.innerHTML = `
...
setTimeout(() => { this.textContent = '⎘ Copy'; this.classList.remove('copied'); });
}, 200
});
return div;
}
...
'',
`Plagiarism Risk Score : ${data.plagiarism_score}/100 (${scoreDesc(data.plagiarism_sco
...
if (!hist.length) {
historyList.innerHTML = '<li class="history-empty">No analyses yet — upload a document to
return;
}
hist.forEach(item => {
...
<span class="hist-badge" style="background:${color}18;color:${color};border:1px solid $
